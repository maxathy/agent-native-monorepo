...
| 16  | Gemini dependency set | `broken` | The retired `text-embedding-004` is gone: embeddings now call `gemini-embedding-001:embedContent` directly with `outputDimensionality` and are L2-normalized (`model/gemini-embedder.ts:38-59`), and the dimension is one constant (`semantic/embedding.ts:19`). **Still `broken` because it is unverified against a live key.** The key in this working tree returns `403 PERMISSION_DENIED — Your API key was reported as leaked`, so the 200 this row needs has not been observed. The retry policy did behave correctly on it: a 4xx failed on the first attempt rather than three. | P2-A |
